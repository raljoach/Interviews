...
public class Solution {
    ...
    private static int test(TreeNode top, int test, int start, int dist,
        ...
        if (expected.size() != list.size()) {
            System.out
                    .println("FAILED: Incorrect number of items in expected vs actual");
            // return;
        } else {
    ...
    private static void getDescendants(TreeNode startNode, int dist,
            Map<TreeNode, TreeNode> backTrack, List<TreeNode> result)
            throws Exception {
    ...
    private static TreeNode findStartNode(TreeNode root, int val,
            Map<TreeNode, TreeNode> backTrack) throws Exception {
        if (root == null) {
            throw new Exception("Root should not be null!");
        }
        if (root.val == val) {
            return root;
        }
