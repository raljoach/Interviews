...
public class Solution {
	...
	private static int test(TreeNode top, int test, int start, int dist, List<Integer> expected) throws Exception {
		...
		if(expected.size()!=list.size())
		{
			...
			for(int i=expected.size()-1; i>=0; i--){
				...
				if(!found){
					System.out.println("FAILED: Following expected item not returned: " + e);
					return test;
				}
				else{
...
private static List<TreeNode> solve(TreeNode root, int val, int dist) throws Exception {
	List<TreeNode> result = find(root,val,dist);
	return result;
}
...
private static List<TreeNode> find(TreeNode root, int val, int dist) {
	...
	for(int i=0; i<currentLevel.size(); i++){
		...
		if(startNode==null && next.val==val){ startNode = next; startLevel = k; }
		if(startNode!=null && currDist==dist){ result.add(next);}
		if(next.left!=null){
			nextLevelCount++;
